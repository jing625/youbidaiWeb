<template>
  <div class="balanceRecord">
    <div class="container">
      ...
      <header>
        <div class="title">
          <h2>总资产(CNY)：{{balanceInfo.balance}} </h2>
        </div>
        <div class="items">
          <div>
            <h1>{{balanceInfo.balanceAvailable}}</h1>
            <h3>可用</h3>
          </div>
          <div>
            <h1>
              {{balanceInfo.balanceLocked}}
            </h1>
            <h3>冻结</h3>
          </div>
        </div>
        <el-table :data="showTableData" stripe style="width: 100%" >
          ...
          <el-table-column prop="money" label="总额">
          </el-table-column>
        ...
        <el-button type="primary" @click="goWithdrawMoney" :disabled="isGrey">提现</el-button>
      </header>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      isGrey() {
        return this.balanceInfo.balanceAvailable === '0.00';
      }
    },
    methods: {
      goAsset() {
      ...
      getBalanceInfo() {
        ...
        if (!this.tableData.finishedAt) {
          ...
          rechargeMoneyDetail().then(res => {
            ...
            for (var i = 0; i < this.balanceInfo.recharges.length; i++) {
              this.tableData.push(this.balanceInfo.recharges[i])
            }
...
<style>
  .balanceRecord header .el-table td {
    padding: 18px 0;
  }
</style>
